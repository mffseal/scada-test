...
def make_udp_fins_read_packets(dmac, smac, dip, sip, dport, sport, address, data):
    s_ethernet, s_ipv4 = make_basic_protocol(dmac, smac, dip, sip)
    s_udp = make_udp(dport, sport)
...
def make_udp_fins_write_packets(dmac, smac, dip, sip, dport, sport, address, data):
    s_ethernet, s_ipv4 = make_basic_protocol(dmac, smac, dip, sip)
    s_udp = make_udp(dport, sport)
    s_ipv4.protocol['value'] = b'\x11'
    s_fins = make_basic_fins()
...
if __name__ == '__main__':
    # spr, dpr = make_udp_fins_read_packets(SMAC, DMAC, SIP, DIP, 9600, 13472, b'\x00\x64', b'\x00\x01\x00\x02')
    # spw, dpw = make_udp_fins_write_packets(SMAC, DMAC, SIP, DIP, 9600, 13472, b'\x00\x64', b'\x00\x01\x00\x02')
    #
    # pkts = []
    # for i in range(1, 50):
    #     if i == 35:
    ...
    #     spr, dpr = make_udp_fins_read_packets(SMAC, DMAC, SIP, DIP, 9600, 13472, b'\x00\x64', data)
    #     pkts.append(spr)
    #     pkts.append(dpr)
    # pcap_wrapper([spw, dpw] + pkts, 'out/fins_test.pcap')

    pcap_maker(make_udp_fins_read_packets, '../out/omron_r.pcap', DMAC, SMAC, DIP, SIP, 9600, 13472, b'\x05')
    pcap_maker(make_udp_fins_write_packets, '../out/omron_w.pcap', DMAC, SMAC, DIP, SIP, 9600, 13472, b'\x05')
